...
import { ChevronLeft, ChevronRight, Github, ArrowLeft } from "lucide-react"
...
interface MissionLogProps {
  ...
  onPrev: () => void
  /** Navigate to Squads & Alliances (right arrow) */
  onNext?: () => void
  /** Return to Tactical Hub */
  onBack: () => void
}

export function MissionLog({ onPrev, onNext, onBack }: MissionLogProps) {
  ...
  return (
    ...
    >
        ...
        >
          ...
          >
            ...
            <div className="relative px-8 py-8 sm:px-10 sm:py-10">
              ...
              >
                AI-powered medical bill classification and automation system built using Python and machine learning techniques for structured document processing.
              </p>

              {/* GitHub Link */}
              <a
                href="https://github.com/adrisha-dev/medibill-ai"
                target="_blank"
                rel="noopener noreferrer"
                className={`group mt-7 inline-flex items-center gap-3 font-mono text-[10px] uppercase tracking-[0.2em] text-danger/60 transition-all duration-500 hover:text-danger sm:text-[11px] ${
                  mounted ? "translate-x-0 opacity-100" : "-translate-x-4 opacity-0"
                }`}
                style={{ transitionDelay: "750ms" }}
              >
                <Github className="h-4 w-4 transition-transform duration-300 group-hover:scale-110" />
                <span className="border-b border-transparent transition-all duration-300 group-hover:border-danger/40">
                  View on GitHub
                </span>
                <span className="inline-block h-px w-0 bg-danger/50 transition-all duration-300 group-hover:w-6" />
              </a>
            </div>
          </div>
        </div>
      ...
      >
        ...
        {/* Right arrow -- to Squads & Alliances */}
        <button
          onClick={handleNext}
          className="group flex cursor-pointer items-center justify-center border border-danger/20 bg-[#0a0a0f]/70 p-3 backdrop-blur-sm transition-all duration-400 hover:border-danger/40 hover:bg-danger/[0.06]"
          style={{
